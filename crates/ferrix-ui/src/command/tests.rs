...
/// Two harnesses on different threads must not share a prefs file.
///
/// This is the integration defect that surfaced when #40 (command palette
/// recency, persisted) merged with #45 (a completed file load persists the
/// recent list). #45 turned every harness test that opens a file into a prefs
/// WRITER, and isolation was via the process-wide `FERRIX_CONFIG_DIR`, so an
/// unrelated test writing prefs truncated the file this test was round-tripping
/// through a simulated restart. It presented as recency coming back empty —
/// a correct feature reporting itself broken, only under a parallel run.
///
/// Asserted the way the bug would actually break it: a second thread writes
/// its own recency WHILE this one holds a value, and this thread's value must
/// survive. Merely asserting a save-then-load round trip on one thread passes
/// against the broken version, since the clobber needs a concurrent writer.
#[test]
fn prefs_are_isolated_per_thread_so_a_concurrent_test_cannot_clobber_them() {
    use crate::prefs::Prefs;

    let dir = std::env::temp_dir().join(format!("ferrix-iso-main-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
    let prev = crate::prefs::set_test_config_dir(Some(dir.clone()));

    let mine = Prefs {
        recent_commands: vec!["view.split".into(), "data.goal_seek".into()],
        ..Prefs::default()
    };
    mine.save().unwrap();

    // A peer test, on its own thread, doing exactly what any harness test that
    // opens a file now does: writing prefs of its own.
    std::thread::spawn(|| {
        let d = std::env::temp_dir().join(format!("ferrix-iso-peer-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&d);
        std::fs::create_dir_all(&d).unwrap();
        crate::prefs::set_test_config_dir(Some(d.clone()));
        let theirs = Prefs {
            recent_commands: vec!["file.open".into()],
            ..Prefs::default()
        };
        theirs.save().unwrap();
        let _ = std::fs::remove_dir_all(&d);
    ...
    assert_eq!(
        Prefs::load().recent_commands,
        vec!["view.split".to_string(), "data.goal_seek".to_string()],
        "a concurrent thread's prefs write landed in this thread's config dir"
    );
